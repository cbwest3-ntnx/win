...
func PdhExpandWildCardPath(szDataSource uintptr, szWildCardPath string, mszExpandedPathList *uint32, pcchPathListLength *uint32, dwFlags *uint32) uint32 {
	ptxt, _ := syscall.UTF16PtrFromString(szWildCardPath)
	ret, _, _ := pdh_ExpandWildCardPath.Call(
		uintptr(szDataSource),
		uintptr(unsafe.Pointer(ptxt)),
